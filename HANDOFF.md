...
- 조작: 이동 방향키/WASD · 발사 `Z`/`Space` · 이탈 `X` · 봄 `C`/`B` · 재시작 `R`.
...
- `BossData` 보스: 코어+파괴가능 파츠, 체력구간 **페이즈 FSM**(조준/n-way/원형 탄막), 기믹 **변신**(스프라이트/색+무적)·**분리**(사망 시 잔해 적), HUD 체력바.
...
- **B2 보스 합체/촉수/다관절 IK** §5.12,14~15.
- **B3 편대 선두추적(snake, leadPath 링버퍼) + leaderBreakScatter** §5.8.
- **B4 W06 록온레이저**(다중 락온→유도 레이저) §3.1 — 남은 무기.

### C. 시스템/정리
- **C1** 난이도 곡선·콤보/배율 스코어링 §7.
- **C2** 풀링 보완(픽업·이펙트·FormationGroup은 현재 Instantiate/Destroy).
- **C3** GDD/CLAUDE.md 동기화(이번 대량 구현 반영).
- **C4** pixellab 스프라이트 확장(탄·아이템·이펙트·추가 적/보스).
